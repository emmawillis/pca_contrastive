...
def main():
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if val["bacc"] > best_bacc:
            best_bacc = val["bacc"]
            best_path.parent.mkdir(parents=True, exist_ok=True)
            torch.save({"model": model.state_dict()}, best_path)
            print(f"  ↳ saved best to {best_path} (BAL-acc={val['bacc']:.4f})")

            _save_val_embeddings(val["embeddings_list"], outdir / "best_embeddings", fname="val.pt")
        ...
        if epoch == args.epochs:
            ...
            print(f"  ↳ saved LAST to {last_path} (BAL-acc={val['bacc']:.4f})")

            _save_val_embeddings(val["embeddings_list"], outdir / "last_embeddings", fname="val.pt")
