...
def run_epoch_triplet(loader, model, triplet_fn, optimizer=None, device="cuda"):
    ...
    with torch.set_grad_enabled(train_mode):
        for batch in loader:
            ...
            _, emb = model(x)          # logits unused here
...
def _stack_embs_labels(emb_list, y_list):
    X = torch.cat([e.cpu() for e in emb_list], 0).numpy() if emb_list else np.empty((0, 0), np.float32)
    y = torch.cat([y.cpu() for y in y_list], 0).numpy() if y_list else np.empty((0,), np.int64)
    return X, y
...
def main():
    ...
    for p in model.parameters():            # unfreeze all
        p.requires_grad = True
    for p in model.encoder.parameters():    # freeze medsam
    ...
    print(f"[triplet] lr_proj={args.lr:g}| triplet_margin={args.triplet_margin:g}")
    ...
    last_path = outdir / "ckpt_last.pt"
    best_bacc = -1

    # -------- loop --------
    for epoch in range(1, args.epochs + 1):
        # 1) Encoder training epoch (triplet)
        tr_loss, train_embeddings, train_labels = run_epoch_triplet(train_loader, model, train_triplet, optimizer=optimizer, device=device)
        va_loss, val_embeddings, val_labels = run_epoch_triplet(val_loader,   model, val_triplet,   optimizer=None,     device=device)
        ...
        X_tr, y_tr = _stack_embs_labels(train_embeddings, train_labels)
        X_va, y_va = _stack_embs_labels(val_embeddings, val_labels)
        ...
        if lr_metrics['bacc'] > best_bacc:
            best_bacc = lr_metrics['bacc']
            best_path.parent.mkdir(parents=True, exist_ok=True)
            torch.save({"model": model.state_dict()}, best_path)
            print(f"  ↳ saved best to {best_path} (BAL-acc={lr_metrics['bacc']:.4f})")

            # after X_va is a NumPy array
            save_dir = outdir / "best_embeddings"
            save_dir.mkdir(parents=True, exist_ok=True)
            torch.save({"embeddings": torch.as_tensor(X_va)}, save_dir / "val.pt")

            print(f"  ↳ saved best val embeddings to {outdir/'best_embeddings'/'val.pt'}")

        if epoch == args.epochs:
            last_path.parent.mkdir(parents=True, exist_ok=True)
            torch.save({"model": model.state_dict()}, last_path)
            print(f"  ↳ saved LAST to {last_path} (BAL-acc={lr_metrics['bacc']:.4f})")

            # after X_va is a NumPy array
            save_dir = outdir / "last_embeddings"
            save_dir.mkdir(parents=True, exist_ok=True)
            torch.save({"embeddings": torch.as_tensor(X_va)}, save_dir / "val.pt")
            print(f"  ↳ saved last val embeddings to {outdir/'last_embeddings'/'val.pt'}")
