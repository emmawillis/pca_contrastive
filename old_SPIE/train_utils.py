...
def build_datasets_and_loaders(
    ...
    train_loader = DataLoader(
        ...
        num_workers=num_workers, pin_memory=pin_memory, collate_fn=collate_resize_to_imgsize
    )
...
def _save_val_embeddings(emb_list, out_dir: Path, fname="val.pt"):
    out_dir.mkdir(parents=True, exist_ok=True)
    embs = torch.cat([e.detach().cpu() for e in emb_list]) if emb_list else torch.empty(0)
    torch.save({"embeddings": embs}, out_dir / fname)
