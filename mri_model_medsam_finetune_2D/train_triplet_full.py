...
def main():
    ...
    if wb is not None:
        # Make Step == epoch for these namespaces
    ...
    # =========================
    # Unfreeze encoder & proj; head unused
    ...
    optimizer_triplet = torch.optim.AdamW(
        [
            {"params": model.proj.parameters(),    "lr": args.triplet_lr, "weight_decay": args.triplet_wd},
            {"params": model.encoder.parameters(), "lr": args.triplet_lr * 0.1, "weight_decay": args.triplet_wd},
        ]
    )
    print(f"[triplet] lr_proj={args.triplet_lr:g} | lr_enc={args.triplet_lr*0.1:g} | margin={args.triplet_margin:g}")
    ...
    for epoch in range(1, args.triplet_epochs + 1):
        ...
        # ---- W&B (aux namespace so it won't clutter overlays) ----
        if wb is not None:
        ...
        # Early stop/select best by LR-on-val BAL-ACC, keep best encoder/proj in memory
    ...
    # =========================
    # Phase 2: Head fine-tuning (classification head, optionally proj)
    # =========================
    # Freeze all; unfreeze head (+ optionally proj)
    for p in model.parameters():
        p.requires_grad = False
    ...
    for epoch in range(1, args.head_epochs + 1):
        ...
        # W&B (unified names for overlays)
        if wb is not None:
            ...
            payload = {
                ...
                "train/loss": tr_loss,
                "train/bacc": tr_bacc,
                # keep a couple of extras tucked under aux
