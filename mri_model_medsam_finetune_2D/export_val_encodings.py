...
def main():
    ...
    ap.add_argument("--num_classes", type=int, default=3)      # isup3 -> 3 classes
    ...
    ap.add_argument("--target", default="isup3", choices=["isup3","isup6"])
    ...
    ap.add_argument("--keep_skipped", action="store_true")  # normally we drop skip==1, like train.py
    ...
    ds = PicaiSliceDataset(
        ...
        use_skip=not args.keep_skipped,
        target=args.target,
        channels=channels,
    ...
    logits = np.concatenate(logits_all, axis=0) if logits_all else np.empty((0, args.num_classes), dtype=np.float32)
    embs   = np.concatenate(embs_all,   axis=0) if embs_all   else np.empty((0, args.proj_dim), dtype=np.float32)
    probs  = torch.softmax(torch.from_numpy(logits), dim=1).numpy() if logits.size else np.empty((0, args.num_classes), dtype=np.float32)
