...
#  2) MedSAM encoder is ALWAYS frozen (never unfrozen, no stage changes)
...
def main():
    ...
    p.add_argument("--outdir", default="./runs/simple_noattn")
    ...
    # New model knobs (default matches what we discussed)
    p.add_argument("--pool_mode", choices=["avg", "max", "avgmax", "gem"], default="avgmax")
    p.add_argument("--gem_p_init", type=float, default=3.0)

    # NOTE: kept for CLI compatibility with the original script, but it is ignored.
    p.add_argument("--freeze_epochs", type=int, default=2,
                   help="(Ignored) Encoder is always frozen in this script.")
    ...
    print("SCRIPT: train_no_attention.py")
    print("ARGS:", args)
    set_seed(args.seed)
    ...
    # Optimizer over only trainable params (proj/head; and pool's p if mode == "gem")
