...
class MedSAMSliceSpatialAttn(nn.Module):
    ...
    def __init__(self,
        ...
        # ---- PROBE CHANNEL ORDER (fixed) ----
        with torch.no_grad():
            feats0 = self.encoder(torch.zeros(1, 3, IMG_SIZE, IMG_SIZE))
            if feats0.dim() != 4:
                raise RuntimeError(f"unexpected encoder output: {tuple(feats0.shape)}")
            # If last dim > second dim, it's channels-last -> permute to [B,C,H,W]
            if feats0.shape[-1] > feats0.shape[1]:
                feats0 = feats0.permute(0, 3, 1, 2).contiguous()
        self.C = feats0.shape[1]
    ...
    def forward(
        ...
        attn_upsample_to: tuple[int, int] | None = None,
    ):
        """
        x: [B,3,H,W] normalized to [0,1]

        Returns:
          - logits [B,num_classes], emb [B,proj_dim]
          - optionally: attn [B,1,Hf,Wf] (or upsampled to attn_upsample_to if provided)
          - optionally: feats [B,C,Hf,Wf] (pre-pool feature map; useful for Grad-CAM)
        """
        ...
        # ---- FIXED: ensure channels-first ----
        if feats.shape[-1] > feats.shape[1]:
            feats = feats.permute(0, 3, 1, 2).contiguous()  # to [B,C,H,W]

        pooled, attn = self.pool(feats)         # [B,C], [B,1,Hf,Wf]
        emb = self.proj(pooled)                 # [B,proj_dim]
        logits = self.head(emb)                 # [B,num_classes]

        # Optional upsample of attention to a target spatial size (e.g., input H×W)
        if return_attn and attn_upsample_to is not None and attn.shape[-2:] != attn_upsample_to:
            attn = F.interpolate(attn, size=attn_upsample_to, mode="bilinear", align_corners=False)

        # Flexible returns (keeps old behavior if both flags are False)
        if return_attn and return_feats:
            return (logits, emb, attn, feats)
        if return_attn:
