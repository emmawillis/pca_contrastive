...
#   --train_mode baseline  : End-to-end CE training (encoder+proj+head) for triplet_epochs+head_epochs epochs
#
...
def _infer_patient_ids_from_df(df, *, patient_col="patient_id", case_col="case_id"):
    ...
    if case_col in cols:
        # Common in PI-CAI style manifests: case_id like "patient_study"
        # We take prefix as patient id guess.
        return df[case_col].astype(str).map(lambda s: s.split("_")[0]).to_numpy()

    # Fallback: try something sane
...
def assert_no_patient_overlap(train_df, val_df, test_df=None, *, patient_col="patient_id", case_col="case_id"):
    ...
    if len(tr_va) or len(tr_te) or len(va_te):
        # Print a small sample to help debug without spamming logs
        def sample(s, k=10):
            s = sorted(list(s))
...
def main():
    ...
    # Triplet phase (only used in two_stage)
    ...
    # Misc
    p.add_argument("--outdir", default="./runs/triplet_then_head")
    ...
    # ==========================================================
    # MODE A: BASELINE = end-to-end CE on encoder+proj+head
    # ==========================================================
    if args.train_mode == "baseline":
        ...
        for epoch in range(1, total_epochs + 1):
            ...
            if wb is not None:
                lrs = [pg.get("lr", None) for pg in optimizer_base.param_groups]
                payload = {
                    "epoch": epoch,
            ...
            if early_base.update(val["bacc"], model, save_path=outdir / "ckpt_baseline_best.pt"):
                print(f"  ↳ [baseline] new best (val BAL-acc={val['bacc']:.4f}) snapshot stored in memory")
            else:
                print(f"  ↳ [baseline] no improvement ({early_base.num_bad}/{early_base.patience})")
                if early_base.num_bad >= early_base.patience:
                    print(f"[baseline] Early stopping at epoch {epoch}.")
                    break

        if not early_base.load_best_into(model, strict=False):
            print("[baseline][warn] No improvement recorded; using last weights.")
        model.to(device).eval()

    # ==========================================================
    # MODE B: TWO_STAGE = triplet alignment then head CE
    # ==========================================================
    else:
        ...
        # =========================
        # Phase 2: CE fine-tuning (scope selectable)
        # =========================
    ...
    if test_loader is not None:
        ...
        if wb is not None:
            payload = {
                "epoch": int(args.triplet_epochs) + int(args.head_epochs),
