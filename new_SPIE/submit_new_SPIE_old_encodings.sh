...
LOGDIR=/home/ewillis/projects/aip-medilab/ewillis/pca_contrastive/new_SPIE/RESULTS_original_encodings/isup0145
mkdir -p "$LOGDIR"
...
OUTDIR_BASE=/home/ewillis/projects/aip-medilab/ewillis/pca_contrastive/new_SPIE/RESULTS_original_encodings/baseline_isup0145
OUTDIR_TRIP=/home/ewillis/projects/aip-medilab/ewillis/pca_contrastive/new_SPIE/RESULTS_original_encodings/triplet_isup0145

HISTO_DIR=/home/ewillis/projects/aip-medilab/shared/picai/histopathology_encodings/efficientnet_b2_embeddings_87acc/correct
HISTO_MARKSHEET=/home/ewillis/projects/aip-medilab/shared/picai/histopathology_encodings/efficientnet_splits
...
#!/bin/bash
#SBATCH -J newSPIE-baseline-isup0145
...
srun -u python -u "$BASELINE_SCRIPT" \
  --use-frozen \
  --medsam_encodings "/home/ewillis/projects/aip-medilab/shared/picai/picai_medsam_zero_shot_encodings/balanced_slice_encodings" \
  --manifest "$MANIFEST" \
  --sam_checkpoint "$CKPT" \
  --target isup0145 \
  --folds_train 1,2,3 \
  ...
  --outdir "$OUTDIR_BASE" \
  --wandb_run_name newSPIE-frozen-baseline-isup0145
EOF
)
...
#!/bin/bash
#SBATCH -J newSPIE-triplet-isup0145
#SBATCH -A aip-medilab
#SBATCH -t 2-00:00:00
#SBATCH -c 8
#SBATCH --mem=64G
...
srun -u python -u "$TRIPLET_SCRIPT" \
  ...
  --provider all \
  --outdir "$OUTDIR_TRIP" \
  --wandb_run_name newSPIE-frozen-triplet-isup0145
EOF
)
